...
func New(numberOfRoutines int, queueCapacity int32) *WorkPool {
	...
	fmt.Println(workPool.queueCapacity, workPool.activeRoutines, workPool.queuedWork)
...
func (workPool *WorkPool) ActiveRoutines() int32 {
	workPool.Lock()
	defer workPool.Unlock()

	return workPool.queueCapacity - workPool.activeRoutines
}
